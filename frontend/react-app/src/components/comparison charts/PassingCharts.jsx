...
const QbCharts = ({player1Statlines, player2Statlines, player1, player2}) => {
    ...
    const getChartData = (statKey) => {
        ...
        for (let i = 0; i < maxLength; i++) {
            if (statKey == "pass_complete") {
                data.push({
                    game: `Game ${i + 1}`,
                    player1: parseFloat(player1Statlines[i]?.[statKey] 
                        / player1Statlines[i]?.pass_attempts).toFixed(2),
                    player2: parseFloat(player2Statlines[i]?.[statKey] 
                        / player2Statlines[i]?.pass_attempts).toFixed(2),
                })
            } else {
                data.push({
                    game: `Game ${i + 1}`,
    ...
    const passYdsData = getChartData("pass_yds");
    const passTdsData = getChartData("pass_tds");
    const compPctData = getChartData("pass_complete");
    const interceptionsData = getChartData("interceptions");
            ...
            {(player1Statlines.length > 0 || player2Statlines.length > 0) && (
                ...
                <div className="flex justify-center space-x-2 h-96 mt-2">
                    ...
                    <div className="flex flex-col justify-center items-center bg-white w-full">
                        <h3 className="text-lg text-gray-500">Passing Yards</h3>
                        <AreaChart width={700} height={350} data={passYdsData} margin={CHART_MARGIN} >
                    ...
                    {/* Passing Touchdowns Chart */}
                    <div className="flex flex-col justify-center items-center bg-white w-full">
                        <h3 className="text-lg text-gray-500">Passing Touchdowns</h3>
                        <BarChart width={700} height={350} data={passTdsData} margin={CHART_MARGIN} >
                ...
                <div className="flex justify-center space-x-2 h-96 mt-2">
                    ...
                    {/* Interceptions Chart */}
                    <div className="flex flex-col justify-center items-center bg-white w-full">
                        <h3 className="text-lg text-gray-500">Interceptions</h3>
                        <BarChart width={700} height={350} data={interceptionsData} margin={CHART_MARGIN} >
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="game" />
                            <YAxis allowDecimals={false} />
                            <Tooltip />
                    ...
                    <div className="flex flex-col justify-center items-center bg-white w-full">
                        <h3 className="text-lg text-gray-500">Completion Percentage</h3>
                        <AreaChart width={700} height={350} data={compPctData} margin={CHART_MARGIN} >
                            <defs><ChartGradients /></defs>
                            <CartesianGrid stroke="#f5f5f5" />
                            <XAxis dataKey="game" />
                            <YAxis />
